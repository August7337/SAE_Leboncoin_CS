...
import GdprDataView from '@/views/account/GdprDataView.vue'
...
vi.mock('axios', () => {
  ...
  const makeGet = (url) => {
    if (typeof url === 'string') {
      ...
      if (lowerUrl.includes('/utilisateurs/') && lowerUrl.includes('/export-rgpd')) {
        return Promise.resolve({ data: {
          profil: { idutilisateur: 1, pseudonyme: 'JohnDoe', email: 'john@example.com' },
          annoncesPubliees: [],
          favoris: [],
...
vi.mock('@/api/axios', () => {
  const makeGet = (url) => {
    if (typeof url === 'string') {
      const lowerUrl = url.toLowerCase()
      if (lowerUrl.includes('/features')) return Promise.resolve({ data: [] })
      if (lowerUrl.includes('/utilisateurs/gdpr')) {
        return Promise.resolve({ data: {
          profil: { idutilisateur: 1, pseudonyme: 'JohnDoe', email: 'john@example.com' },
          annoncesPubliees: [],
          favoris: [],
...
function createWrapper() {
  return shallowMount(GdprDataView, {
    global: {
