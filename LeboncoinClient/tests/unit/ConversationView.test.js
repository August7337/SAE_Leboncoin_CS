...
vi.mock('@/api/axios', () => {
  const makeGet = (url) => {
    if (typeof url === 'string') {
      const lowerUrl = url.toLowerCase()
      if (lowerUrl.includes('/features')) return Promise.resolve({ data: [] })
      if (lowerUrl.includes('/messages/chat/')) {
        return Promise.resolve({ data: {
          interlocuteur: { pseudonyme: 'Interlocuteur', email: 'interlocuteur@test.com' },
          messages: [
            { id: 1, expediteurId: 1, destinataireId: 1, contenu: 'Bonjour' },
          ],
        } })
      }
      if (lowerUrl.includes('/annonces/') && lowerUrl.endsWith('/similaires')) return Promise.resolve({ data: [] })
  ...
  const makeGet = (url) => {
    if (typeof url === 'string') {
      ...
      if (lowerUrl.includes('/messages/chat/')) {
        return Promise.resolve({ data: {
          interlocuteur: { pseudonyme: 'Interlocuteur', email: 'interlocuteur@test.com' },
          messages: [{ id: 1, expediteurId: 1, destinataireId: 1, contenu: 'Bonjour' }],
        } })
      }
      if (lowerUrl.includes('/annonces/') && lowerUrl.endsWith('/similaires')) return Promise.resolve({ data: [] })
      if (lowerUrl.includes('/annonces/')) {
...
function createWrapper() {
  return shallowMount(ConversationView, {
    global: {
