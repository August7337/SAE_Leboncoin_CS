  ...
  <div class="bg-[#f8f9fb] min-h-screen pb-12">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <!-- Mes Annonces -->
        <router-link v-if="authState.hasPermission('app.view.my_annonces')" to="/my-annonces" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex items-start gap-4">
        ...
        <!-- Mes Réservations -->
        <router-link v-if="authState.hasPermission('app.view.my_reservations')" to="/my-reservations" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex items-start gap-4">
        ...
        <!-- Mes Litiges -->
        <router-link v-if="authState.hasPermission('app.view.my_incidents')" to="/my-incidents" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex items-start gap-4">
          ...
          <div>
            <h2 class="text-lg font-bold text-gray-900">Mes Litiges</h2>
            <p class="text-gray-500 text-sm mt-1">Suivre vos dossiers en cours</p>
          </div>
        </router-link>

        <!-- Favoris -->
        <router-link v-if="authState.hasPermission('app.view.favorites')" to="/favorites" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex items-start gap-4">
          <div class="w-10 h-10 flex-shrink-0 bg-red-50 rounded-lg flex items-center justify-center text-red-600">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        ...
        <!-- Données RGPD -->
        <router-link v-if="authState.hasPermission('app.view.gdpr_data')" to="/mes-donnees-personnelles" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow flex items-start gap-4">
          <div class="w-10 h-10 flex-shrink-0 bg-teal-50 rounded-lg flex items-center justify-center text-teal-600">
             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
          ...
          <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Gérer vos données personnelles</p>
          </div>
        </router-link>
