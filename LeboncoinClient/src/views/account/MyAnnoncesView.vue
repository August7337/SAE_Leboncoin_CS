...
async function handleDelete() {
  ...
  try {
    await axios.delete(`https://localhost:7057/api/Annonces/${annonceToDelete.value.idannonce}`)
  ...
  <div class="bg-[#f8f9fb] min-h-screen py-8">
          ...
          <div v-else class="space-y-4">
            ...
            >
              ...
              <div class="flex-grow flex flex-col justify-between">
                ...
                <!-- Reservations Summary (Mocked logic based on Laravel structure) -->
                <div class="mt-4 border-t border-gray-100 pt-3">
                  ...
                  <div v-else class="mt-2 space-y-2 max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                    <div 
                      ...
                      class="bg-white/70 border border-gray-200 rounded-lg px-3 py-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 hover:border-orange-200 hover:shadow-sm transition"
                      :class="{ 'opacity-60 bg-gray-50': res.estPassee }"
                    >
                      <div class="flex items-center gap-2 text-sm font-semibold text-gray-900">
                        <span>{{ res.prenomclient }} {{ res.nomclient }}</span>
                        <span v-if="res.estPassee" class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-gray-200 text-gray-700">
                          Passée
                        </span>
                ...
                <div class="flex items-center gap-6 mt-4 pt-3 border-t border-gray-100 text-sm font-bold">
                  ...
                  <button class="text-gray-700 hover:text-blue-600 flex items-center gap-1 transition group cursor-not-allowed opacity-50">
    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200">
        ...
        <div v-if="annonceToDelete?.reservations?.length > 0" class="bg-red-50 border border-red-200 rounded-lg p-3 mb-6">
          <p class="text-sm text-red-800 font-semibold">
            <svg class="w-4 h-4 inline mr-1" fill="currentColor" viewBox="0 0 20 20">
              <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
            </svg>
            Attention : cette annonce posséde des réservations.
          </p>
        </div>
