<template>
  ...
  <div class="register-container relative">
    <button type="button" class="back-arrow-btn" @click="goBack" title="Retour à l'étape 1">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="h-6 w-6"
        fill="none"
...
const submitFinal = async () => {
  ...
  const payload = {
    ...
    codePostal:
      userStep1.codePostal ||
      userStep1.adresseUtilisateur?.codePostal ||
      userStep1.codePostal ||
      '',
  ...
  try {
    ...
    if (error.response && error.response.status === 409) {
      ...
      const isStep1Error =
        step1Fields.includes(target.toLowerCase()) || step1Fields.includes(target)
