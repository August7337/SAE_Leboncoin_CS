  ...
  <div class="register-container relative">
    ...
    <form @submit.prevent="submitFinal">
      ...
      <div class="field">
        ...
        <input v-model="proForm.numsiret" type="text" maxlength="14" :class="{ 'error': errors.numsiret }" />
...
const validatePro = () => {
  ...
  let isValid = true
  if (!proForm.nomsociete.trim()) { errors.nomsociete = "Requis."; isValid = false; }
  if (!/^\d{14}$/.test(proForm.numsiret)) { errors.numsiret = "14 chiffres requis."; isValid = false; }
  return isValid
}
...
const submitFinal = async () => {
  ...
  try {
    const response = await axios.post("https://localhost:7057/api/Utilisateurs/register-professionnel", apiPayload)
