        ...
        <div class="relative w-full h-64 md:w-80 md:h-52 flex-shrink-0 group">
          ...
          >
            <template v-if="annonce.photos?.length > 0">
              <div v-for="(photo, index) in annonce.photos" :key="index" class="min-w-full h-full snap-start overflow-hidden">
                <img :src="photo.lienphoto" class="w-full h-full object-cover" loading="lazy" />
              </div>
            </template>
            <div v-else class="min-w-full h-full snap-start bg-gray-200 flex items-center justify-center">
          ...
          <div class="absolute bottom-3 w-full flex justify-center gap-1.5 z-10">
            <div
              ...
              :class="initCarouselIndex(annonce.idannonce) === index ? 'bg-white w-2 h-2' : 'bg-white/50 w-1.5 h-1.5'"
            ></div>
          </div>
          ...
          <div class="absolute top-2 right-2 z-20">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 transition-colors" :class="isFavorite(annonce.idannonce) ? 'text-red-500 fill-red-500' : 'text-gray-600'">
        ...
        >
          <div>
            <div class="flex items-center justify-between w-full mb-1">
              <h2 class="text-lg font-bold text-gray-900 leading-tight">{{ annonce.titreannonce }}</h2>
              <div class="flex items-center bg-gray-100 px-2 py-0.5 rounded-md">
                <svg class="w-3.5 h-3.5 text-orange-600 mr-1" fill="currentColor" viewBox="0 0 24 24">
